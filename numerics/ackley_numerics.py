...
if __name__ == "__main__":
    ...
    if load_res:
        ...
        main_config = {
            ...
            "h": [1, 1e-1, 1e-2, 1e-3],
            "d": [4, 8, 9, 10], 
            "N_RBF": list(int(x) for x in np.logspace(2, 4, 11))
        }
        ...
        points_cache, rbf_cache = pre_compute_pts(Ackley, configs, N)
        ...
        with open('numerics/results/ackley_res.pkl', 'wb') as f:
            pickle.dump([main_config, N, df, df_quad], f)

    plot_figs_N_RBF("Ackley", df, "numerics")
